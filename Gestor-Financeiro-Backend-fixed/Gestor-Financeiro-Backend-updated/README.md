...
Este projeto inclui testes automáticos de integração (Mocha + Supertest) para validar as rotas:
- Categories
- Account
- Spendings

Os testes usam uma base de dados **separada** para não afetar a BD principal.

### Pré-requisitos
- Node.js instalado
- MariaDB instalado e a correr
- Porta da MariaDB: **3308** (ajusta se necessário)

### Base de dados de testes
Criar a BD de testes e as tabelas (exemplo):
- BD: `gestor_db_test`
- Tabelas: `utilizador`, `categorias`, `conta`, `gastos`, `chaves`

### Utilizador de testes (recomendado)
No MariaDB:
```sql
...
### Boa prática (recomendo fazer)
Cria também um ficheiro **`.env.integration.example`** (sem password real), para poderes commitar:
...
DB_NAME=gestor_db_test
PORT=3001
SESSION_SECRET=integration_secret
