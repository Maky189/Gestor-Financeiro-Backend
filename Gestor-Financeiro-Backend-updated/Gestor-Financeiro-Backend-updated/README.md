...
## Testes automáticos (integração com MariaDB)

Os testes automáticos usam **uma base de dados separada** para não afetar a BD principal.

- BD de testes: `gestor_db_test`
- Porta MariaDB (exemplo neste projeto): `3308`
- Os testes fazem **reset das tabelas** antes de cada teste (isolamento).

### Pré-requisitos
- Node.js
- MariaDB a correr na porta configurada (ex: `3308`)

### Criar BD e tabelas de testes
1. Cria o ficheiro `sql/Db.test.sql` (igual ao `sql/Db.sql`, mas a apontar para `gestor_db_test`)
2. Importa no MariaDB.

### Utilizador de BD para testes (recomendado)
No MariaDB:

```sql
CREATE USER IF NOT EXISTS 'testuser'@'localhost' IDENTIFIED BY 'testpass';
GRANT ALL PRIVILEGES ON gestor_db_test.* TO 'testuser'@'localhost';
FLUSH PRIVILEGES;
```

### Configurar variáveis para testes
Este repositório inclui um exemplo: `.env.integration.example`.

1) Copia para `.env.integration` e ajusta `DB_PASSWORD`:
...
> **Não faças commit** do `.env.integration` (contém credenciais).  
> O `.env.integration.example` pode (e deve) ser commitado.

### Instalar dependências e correr testes
